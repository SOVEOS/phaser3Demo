  ...
  game.shouldUpdate = false; // flag to ensure the game is not updating automatically

  game.loop.update = function (time, delta) {
    if (game.shouldUpdate) {
        game.step(time, delta);
        game.shouldUpdate = false;
      }
    }; // function to update the game if the game.shouldUpdate = true
      ...
      for (let y = 0; y < mapHeight; y++) {
          for (let x = 0; x < mapWidth; x++) {
              if (y % 2 === 0 && x % 2 === 0) {
                  ...
                  tile.setTint(Math.random() * 0xffffff);
                  layer.add(tile);
                  tileCounter++;
              } else if (y % 2 === 1 && x % 2 === 1) {
                  const tile = this.add.image(x * tileWidth / 2, y * offsetY, 'tile00')
